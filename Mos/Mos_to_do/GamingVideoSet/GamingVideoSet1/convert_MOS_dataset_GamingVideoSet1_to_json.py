...
def decode_line(line):
    ...
    PVS_params['fps'] = int(re.search(r'\d+', v_pvs[1]).group())  # Search for the first sequence of digits
    PVS_params['duration'] = int(re.search(r'\d+', v_pvs[2]).group())  # Search for the first sequence of digits
    PVS_params['frames_tot'] = PVS_params['fps'] * PVS_params['duration']
...
csv_information['dataset_name'] = 'GamingVideoSET'
...
# Save to JSON file
json_file_path = "Scores_GamingVideoSet1.json"  # Specify your desired output file name
