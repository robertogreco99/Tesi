...
if not os.path.isfile(csv_filename):
       ...
       #add void columns for models and features
        ...
        for param_string in essim_param_string_list:
            for essim_feature in essim_features:
                ...
                row[col_name] = -1  # Imposta il valore della colonna su -1 (o il valore desiderato)
    ...
    if distorted_video not in df_existing['Distorted_file_name'].values:
            ...
            for param_string in essim_param_string_list:
                for essim_feature in essim_features:
                    ...
                    if col_name not in df_existing.columns:
                        df_existing[col_name] = -1  # Aggiungi la colonna con valore iniziale -1
                        row[col_name] = -1  # Imposta il valore nella riga
        ...
        df_existing = pd.read_csv(csv_filename)
        # Aggiungi le colonne mancanti per ogni combinazione di essim_feature e param_string
        for param_string in essim_param_string_list:
            for essim_feature in essim_features:
                col_name = f"{essim_feature}_{param_string.strip()}"
                if col_name not in df_existing.columns:
...
essim_file_names = []
# Ciclo per generare un filename per ogni parametro
for string_i in essim_param_string_list:
    # Genera il nome del file utilizzando il parametro attuale
    if dataset in ["KUGVD", "GamingVideoSet1", "GamingVideoSet2"]:
        if width_old != '1920' or height_old != '1080':
    ...
    # Aggiungi il filename alla lista
...
if (use_essim == "True" and use_libvmaf == "True"):
    ...
    essim_dframes_list = []
    # Associa i file ai parametri corrispondenti
    ...
    # Inizializza il DataFrame con 'dframes'
    merged_df = dframes.copy()

    # Esegui un merge progressivo con ogni DataFrame in essim_dframes_list
